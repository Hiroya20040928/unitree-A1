...
- When the PC reports mode=lost, this node takes over and writes /tmp/a1_follow_cmd_raw; body safety filter writes /tmp/a1_follow_cmd.
...
Run on NX:
  ...
  python ~/a1_slam_route_fallback_node_v6_5_passive.py _scan_topic:=/scan _odom_topic:=/odom
...
FOLLOW_CMD_PATH = "/tmp/a1_follow_cmd_raw"
...
def main():
    rospy.init_node("a1_slam_route_fallback_node_v6_5_passive")
    ...
    rospy.loginfo("a1_slam_route_fallback_node_v6_5_passive started scan=%s odom=%s", scan_topic, odom_topic)
    main_loop()
