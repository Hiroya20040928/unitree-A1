...
echo "[A1 START v6.4] run_id=$RUN_ID"
echo "[A1 START v6.4] logs=$LOG_DIR"
echo "[A1 START v6.4] body_filter: BODY_VY_BIAS=$BODY_VY_BIAS BODY_VY_SIGN=$BODY_VY_SIGN BODY_WZ_BIAS=$BODY_WZ_BIAS BODY_WZ_SIGN=$BODY_WZ_SIGN"
...
require_file() {
  local f="$1"
  if [ ! -f "$f" ]; then
    echo "[A1 START v6.4] ERROR: required file missing: $f" >&2
    exit 1
  fi
}
...
echo "[A1 START v6.4] checking required files"
require_file "$HOME/a1_laserscan_obstacle_writer.py"
require_file "$HOME/a1_follow_lowlatency_depth_server_raw.py"
require_file "$HOME/a1_slam_route_fallback_node_v6_4_raw.py"
require_file "$HOME/a1_body_safety_filter.py"

DRIVER_BIN="$HOME/unitree_legged_sdk/build/a1_high_follow_driver"
if [ ! -x "$DRIVER_BIN" ]; then
  echo "[A1 START v6.4] ERROR: original safe driver not found: $DRIVER_BIN" >&2
  echo "[A1 START v6.4] This version intentionally uses the original driver because its obstacle stop was already confirmed." >&2
  exit 1
fi

echo "[A1 START v6.4] sudo check"
sudo -v

echo "[A1 START v6.4] stopping old processes"

echo "[A1 START v6.4] force kill old Unitree drivers"
sudo pkill -9 -f a1_high_follow_driver 2>/dev/null || true
sudo pkill -9 -f a1_high_follow_driver 2>/dev/null || true
pkill -9 -f base_controller_node 2>/dev/null || true
pkill -9 -f lcm_server_high 2>/dev/null || true
pkill -9 -f example_walk 2>/dev/null || true
sleep 1
...
pkill -f a1_body_safety_filter.py 2>/dev/null || true
pkill -f a1_follow_lowlatency_depth_server_raw.py 2>/dev/null || true
pkill -f example_walk 2>/dev/null || true
sleep 1

echo "[A1 START v6.4] reset eth0"
...
echo "[A1 START v6.4] start SLAM fresh"
...
echo "[A1 START v6.4] waiting for /scan"
for i in $(seq 1 75); do
  ...
  sleep 1
  if [ "$i" = "75" ]; then
    echo "[A1 START v6.4] ERROR: /scan did not appear" >&2
    tail -120 "$LOG_DIR/slam_planner_online.log" >&2 || true
    exit 1
  fi
done

echo "[A1 START v6.4] kill competing A1 control nodes"
pkill -f base_controller_node 2>/dev/null || true
pkill -f lcm_server_high 2>/dev/null || true
pkill -f example_walk 2>/dev/null || true
sleep 1

echo "[A1 START v6.4] clear current map if service exists"
...
echo "[A1 START v6.4] check /scan message"
run_ros "timeout 8 rostopic echo -n 1 /scan" > "$LOG_DIR/scan_first_msg.log" 2>&1 || {
  echo "[A1 START v6.4] ERROR: /scan exists but no message" >&2
  tail -80 "$LOG_DIR/scan_first_msg.log" >&2 || true
  exit 1
}

echo "[A1 START v6.4] start obstacle writer"
...
echo "[A1 START v6.4] start body safety filter"
rm -f /tmp/a1_follow_cmd /tmp/a1_follow_cmd_raw /tmp/a1_body_safety_debug
nohup bash --noprofile --norc -c "source /opt/ros/melodic/setup.bash; source $HOME/catkin_ws/devel/setup.bash; export ROS_MASTER_URI=http://localhost:11311; export ROS_IP=192.168.12.1; python $HOME/a1_body_safety_filter.py _scan_topic:=/scan _front_stop_m:=0.70 _front_slow_m:=1.05 _side_hard_m:=0.28 _side_soft_m:=0.45 _rear_side_hard_m:=0.30 _rear_side_soft_m:=0.48 _desired_side_m:=0.60 _k_side_vy:=0.055 _max_side_vy:=0.045 _vy_bias:=${BODY_VY_BIAS} _vy_sign:=${BODY_VY_SIGN} _wz_bias:=${BODY_WZ_BIAS} _wz_sign:=${BODY_WZ_SIGN} _max_vx_when_side_close:=0.090 _narrow_corridor_m:=0.72 _rear_swing_protect:=false" > "$LOG_DIR/body_safety_filter.log" 2>&1 &
echo $! > "$LOG_DIR/body_safety_filter.pid"

ROUTE_NODE="$HOME/a1_slam_route_fallback_node_v6_4_raw.py"
echo "[A1 START v6.4] start route fallback node: $ROUTE_NODE"
...
echo "[A1 START v6.4] waiting for route fallback HTTP"
ROUTE_OK=0
...
if [ "$ROUTE_OK" != "1" ]; then
  echo "[A1 START v6.4] ERROR: route fallback HTTP not responding" >&2
  tail -120 "$LOG_DIR/route_fallback.log" >&2 || true
  exit 1
fi

echo "[A1 START v6.4] start camera/follow HTTP server"
nohup bash --noprofile --norc -c "python3 -u $HOME/a1_follow_lowlatency_depth_server_raw.py" > "$LOG_DIR/camera_server.log" 2>&1 &
echo $! > "$LOG_DIR/camera_server.pid"

echo "[A1 START v6.4] waiting for camera HTTP"
CAM_OK=0
...
if [ "$CAM_OK" != "1" ]; then
  echo "[A1 START v6.4] ERROR: camera HTTP server not responding" >&2
  echo "[A1 START v6.4] camera log:" >&2
  tail -160 "$LOG_DIR/camera_server.log" >&2 || true
  echo "[A1 START v6.4] device summary:" >&2
...
echo "[A1 START v6.4] start Unitree driver: $DRIVER_BIN"
...
echo "[A1 START v6.4] process summary"
