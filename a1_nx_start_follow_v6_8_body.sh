...
BODY_FILTER="$HOME/a1_lidar_footprint_filter_v6_8.py"
...
require_file() {
  if [ ! -f "$1" ]; then
    echo "[A1 START v6.8] ERROR: missing $1" >&2
    exit 1
  fi
}
...
if [ ! -x "$DRIVER_BIN" ]; then
  echo "[A1 START v6.8] ERROR: driver not executable: $DRIVER_BIN" >&2
  echo "[A1 START v6.8] check: ls -lh $DRIVER_BIN ; file $DRIVER_BIN" >&2
  exit 1
fi

printf '[A1 START v6.8] run_id=%s\n' "$RUN_ID"
printf '[A1 START v6.8] logs=%s\n' "$LOG_DIR"
printf '[A1 START v6.8] BODY_FILTER_MODE=%s\n' "$BODY_FILTER_MODE"
printf '[A1 START v6.8] scan provider=%s %s\n' "$SCAN_LAUNCH_PKG" "$SCAN_LAUNCH_FILE"
printf '[A1 START v6.8] footprint body=%sx%s lidar_x=%s dyn_front=%s dyn_rear=%s dyn_side=%s margin=%s\n' "$BODY_LENGTH_M" "$BODY_WIDTH_M" "$LIDAR_X_FROM_BODY_CENTER_M" "$DYNAMIC_FRONT_EXTRA_M" "$DYNAMIC_REAR_EXTRA_M" "$DYNAMIC_SIDE_EXTRA_M" "$SAFETY_MARGIN_M"
printf '[A1 START v6.8] front stop=%s turn_only=%s slow=%s half_deg=%s\n' "$FRONT_STOP_M" "$FRONT_TURN_ONLY_M" "$FRONT_SLOW_M" "$FRONT_SECTOR_HALF_DEG"

sed -i 's/\bsouce\b/source/g' "$HOME/.bash_profile" "$HOME/.bashrc" 2>/dev/null || true

echo "[A1 START v6.8] sudo check"
sudo -v

echo "[A1 START v6.8] stopping old processes"
...
echo "[A1 START v6.8] start scan provider"
nohup bash --noprofile --norc -c "source /opt/ros/melodic/setup.bash; source $HOME/catkin_ws/devel/setup.bash; export ROS_MASTER_URI=http://localhost:11311; export ROS_IP=192.168.12.1; roslaunch $SCAN_LAUNCH_PKG $SCAN_LAUNCH_FILE" > "$LOG_DIR/scan_provider.log" 2>&1 &
echo $! > "$LOG_DIR/scan_provider.pid"

echo "[A1 START v6.8] waiting for /scan"
for i in $(seq 1 60); do
  ...
  sleep 1
  if [ "$i" = "60" ]; then
    echo "[A1 START v6.8] ERROR: /scan did not appear" >&2
...
echo "[A1 START v6.8] check /scan message"
run_ros "timeout 8 rostopic echo -n 1 /scan" > "$LOG_DIR/scan_first_msg.log" 2>&1 || {
  echo "[A1 START v6.8] ERROR: /scan exists but no message" >&2
  tail -120 "$LOG_DIR/scan_first_msg.log" >&2 || true
  exit 1
}

echo "[A1 START v6.8] initialize command files"
...
if [ "$BODY_FILTER_MODE" = "pass" ]; then
  echo "[A1 START v6.8] BODY_FILTER_MODE=pass: direct passthrough, not recommended"
...
EOF
  ...
  echo "[A1 START v6.8] start body footprint filter monitor_only=$MONITOR_ONLY"
...
if [ "$CAM_OK" != "1" ]; then
  echo "[A1 START v6.8] ERROR: camera HTTP server not responding" >&2
  tail -160 "$LOG_DIR/camera_server.log" >&2 || true
  exit 1
fi

echo "[A1 START v6.8] start Unitree driver: $DRIVER_BIN"
...
if ! pgrep -f '[a]1_high_follow_driver' >/dev/null 2>&1; then
  echo "[A1 START v6.8] ERROR: Unitree driver is not running" >&2
  tail -120 "$LOG_DIR/high_follow_driver.log" >&2 || true
  exit 1
fi

echo "[A1 START v6.8] process summary"
ps aux | grep -E 'slamware_ros_sdk_server_node|a1_lidar_footprint_filter_v6_8|a1_follow_lowlatency_depth_server|a1_high_follow_driver|a1_cmd_passthrough_loop' | grep -v grep || true

echo "[A1 START v6.8] status files"
echo "  logs: $LOG_DIR"
...
echo "[A1 START v6.8] DONE. Start PC client next."
