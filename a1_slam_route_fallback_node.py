...
def compute_route_cmd(scan, pose, lost_age, lost_dist, last_xerr):
    ...
    max_vx = rospy.get_param("~route_max_vx", 0.075)
    align_vx = rospy.get_param("~route_align_vx", 0.030)
    max_wz = rospy.get_param("~route_max_wz", 0.38)
    k_heading = rospy.get_param("~k_heading", 1.35)
    heading_sign = rospy.get_param("~heading_sign", 1.0)
    side_pref_gain = rospy.get_param("~side_pref_gain", 0.42)
    heading_penalty = rospy.get_param("~heading_penalty", 0.35)
    ...
    desired_side = rospy.get_param("~desired_side_m", 0.75)
    k_side_vy = rospy.get_param("~k_side_vy", 0.080)
    k_wall_yaw = rospy.get_param("~k_wall_yaw", 0.10)
    vy_bias = rospy.get_param("~vy_bias", -0.055)
...
def main_loop():
    ...
    max_lost_time = rospy.get_param("~max_lost_time", 3.0)
    max_lost_dist = rospy.get_param("~max_lost_dist", 0.85)
    state_timeout = rospy.get_param("~state_timeout", 1.0)
    alpha = rospy.get_param("~cmd_alpha", 0.35)
