...
FollowCmd A1HighFollow::readFollowCmd()
{
    ...
    fc.vy = clampf(fc.vy, -0.10f, 0.10f);   // lateral drift / wall-centering correction
    fc.wz = clampf(fc.wz, -0.50f, 0.50f);   // allow in-place turning in corners
...
void A1HighFollow::RobotControl()
{
    ...
    if (!warmup && ok && fc.valid && fc.enable) {
        ...
        // LiDAR front safety policy:
        // - very close (<0.48 m): safetyOK() has already forced full stop.
        // - near front obstacle: do not walk forward, but allow yaw/side correction so the robot can turn out of corners.
        float front = readFrontObstacleM();
        if (front < 0.85f) {
            target_vx = 0.0f;
            if (std::fabs(target_wz) < 0.08f) target_wz = 0.0f;
        }
    }

    // Smooth commands. Prevent sudden step commands from vision jitter.
    cur_vx = slew(cur_vx, target_vx, 0.0025f);
    cur_vy = slew(cur_vy, target_vy, 0.0020f);
    cur_wz = slew(cur_wz, target_wz, 0.0055f);
...
int main(void)
{
    std::cout << "A1 High-level person-follow driver v2 route-safe" << std::endl;
