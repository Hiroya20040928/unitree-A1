...
require_file() {
  local f="$1"
  if [ ! -f "$f" ]; then
    echo "[A1 START] ERROR: required file missing: $f" >&2
    exit 1
  fi
}
...
if [ ! -x "$DRIVER_BIN" ]; then
  echo "[A1 START] ERROR: high follow driver binary not found" >&2
  echo "[A1 START] expected: ~/unitree_legged_sdk/build/a1_high_follow_driver_v2" >&2
  echo "[A1 START] fallback:  ~/unitree_legged_sdk/build/a1_high_follow_driver" >&2
  exit 1
fi
...
echo "[A1 START] check /scan message"
...
if [ "$ROUTE_OK" != "1" ]; then
  echo "[A1 START] ERROR: route fallback HTTP not responding" >&2
...
echo "[A1 START] waiting for camera HTTP"
CAM_OK=0
...
if [ "$CAM_OK" != "1" ]; then
  ...
  echo "[A1 START] camera log:" >&2
  tail -160 "$LOG_DIR/camera_server.log" >&2 || true
  echo "[A1 START] device summary:" >&2
