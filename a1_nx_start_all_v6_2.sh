...
echo "[A1 START v6.2] run_id=$RUN_ID"
echo "[A1 START v6.2] logs=$LOG_DIR"
...
require_file() {
  local f="$1"
  if [ ! -f "$f" ]; then
    echo "[A1 START v6.2] ERROR: required file missing: $f" >&2
    exit 1
  fi
}
...
if [ ! -x "$DRIVER_BIN" ]; then
  echo "[A1 START v6.2] ERROR: high follow driver binary not found" >&2
  echo "[A1 START v6.2] expected: ~/unitree_legged_sdk/build/a1_high_follow_driver_v2" >&2
  echo "[A1 START v6.2] fallback:  ~/unitree_legged_sdk/build/a1_high_follow_driver" >&2
  exit 1
fi

echo "[A1 START v6.2] sudo check"
sudo -v

echo "[A1 START v6.2] stopping old processes"
...
echo "[A1 START v6.2] start SLAM fresh"
...
echo "[A1 START v6.2] waiting for /scan"
for i in $(seq 1 75); do
  ...
  sleep 1
  if [ "$i" = "75" ]; then
    echo "[A1 START v6.2] ERROR: /scan did not appear" >&2
    tail -120 "$LOG_DIR/slam_planner_online.log" >&2 || true
    exit 1
  fi
done

echo "[A1 START v6.2] kill competing A1 control nodes"
pkill -f base_controller_node 2>/dev/null || true
pkill -f lcm_server_high 2>/dev/null || true
pkill -f example_walk 2>/dev/null || true
sleep 1

echo "[A1 START v6.2] clear current map if service exists"
...
echo "[A1 START v6.2] check /scan message"
run_ros "timeout 8 rostopic echo -n 1 /scan" > "$LOG_DIR/scan_first_msg.log" 2>&1 || {
  echo "[A1 START v6.2] ERROR: /scan exists but no message" >&2
  tail -80 "$LOG_DIR/scan_first_msg.log" >&2 || true
  exit 1
}

echo "[A1 START v6.2] start obstacle writer"
...
ROUTE_NODE="$HOME/a1_slam_route_fallback_node_v6_2.py"
echo "[A1 START v6.2] start route fallback node: $ROUTE_NODE"
nohup bash --noprofile --norc -c "source /opt/ros/melodic/setup.bash; source $HOME/catkin_ws/devel/setup.bash; export ROS_MASTER_URI=http://localhost:11311; export ROS_IP=192.168.12.1; python '$ROUTE_NODE' _scan_topic:=/scan _odom_topic:=/odom _max_lost_time:=2.4 _max_lost_dist:=0.65 _route_max_vx:=0.045 _route_align_vx:=0.020 _route_max_wz:=0.16 _k_heading:=0.40 _desired_side_m:=0.65 _k_side_vy:=0.025 _k_wall_yaw:=0.02 _vy_bias:=0.000 _cmd_alpha:=0.18" > "$LOG_DIR/route_fallback.log" 2>&1 &
echo $! > "$LOG_DIR/route_fallback.pid"

echo "[A1 START v6.2] waiting for route fallback HTTP"
ROUTE_OK=0
...
if [ "$ROUTE_OK" != "1" ]; then
  echo "[A1 START v6.2] ERROR: route fallback HTTP not responding" >&2
  tail -120 "$LOG_DIR/route_fallback.log" >&2 || true
  exit 1
fi

echo "[A1 START v6.2] start camera/follow HTTP server"
...
echo "[A1 START v6.2] waiting for camera HTTP"
CAM_OK=0
...
if [ "$CAM_OK" != "1" ]; then
  echo "[A1 START v6.2] ERROR: camera HTTP server not responding" >&2
  echo "[A1 START v6.2] camera log:" >&2
  tail -160 "$LOG_DIR/camera_server.log" >&2 || true
  echo "[A1 START v6.2] device summary:" >&2
...
echo "[A1 START v6.2] start Unitree driver: $DRIVER_BIN"
...
echo "[A1 START v6.2] process summary"
