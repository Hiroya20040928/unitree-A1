...
SIDE_HARD_CLEARANCE_M="${SIDE_HARD_CLEARANCE_M:-0.03}"
SIDE_SOFT_CLEARANCE_M="${SIDE_SOFT_CLEARANCE_M:-0.10}"
DESIRED_SIDE_CLEARANCE_M="${DESIRED_SIDE_CLEARANCE_M:-0.15}"
REAR_HARD_CLEARANCE_M="${REAR_HARD_CLEARANCE_M:-0.04}"
REAR_SOFT_CLEARANCE_M="${REAR_SOFT_CLEARANCE_M:-0.12}"

MAX_VX="${MAX_VX:-0.16}"
MAX_WZ="${MAX_WZ:-0.45}"
MAX_WZ_NEAR="${MAX_WZ_NEAR:-0.25}"
MAX_VX_SIDE_SOFT="${MAX_VX_SIDE_SOFT:-0.10}"
MAX_VX_SIDE_HARD="${MAX_VX_SIDE_HARD:-0.05}"

# Known working scan provider in this A1 image. It may launch nodes whose package names contain "slam",
# but this follow program consumes only /scan. It ignores /map, /odom, goals, planners, and route fallback.
SCAN_IP="${SCAN_IP:-192.168.11.1}"
SCAN_LAUNCH_PKG="${SCAN_LAUNCH_PKG:-slam_planner}"
SCAN_LAUNCH_FILE="${SCAN_LAUNCH_FILE:-slam_planner_online.launch}"
...
BODY_FILTER="$HOME/a1_lidar_footprint_filter_v6_8_2.py"
...
require_file() {
  if [ ! -f "$1" ]; then
    echo "[A1 START v6.8.2] ERROR: missing $1" >&2
    exit 1
  fi
}
...
if [ ! -x "$DRIVER_BIN" ]; then
  echo "[A1 START v6.8.2] ERROR: driver not executable: $DRIVER_BIN" >&2
  echo "[A1 START v6.8.2] check: ls -lh $DRIVER_BIN ; file $DRIVER_BIN" >&2
  exit 1
fi

printf '[A1 START v6.8.2] run_id=%s\n' "$RUN_ID"
printf '[A1 START v6.8.2] logs=%s\n' "$LOG_DIR"
printf '[A1 START v6.8.2] BODY_FILTER_MODE=%s\n' "$BODY_FILTER_MODE"
printf '[A1 START v6.8.2] scan provider=%s %s\n' "$SCAN_LAUNCH_PKG" "$SCAN_LAUNCH_FILE"
printf '[A1 START v6.8.2] footprint body=%sx%s lidar_x=%s dyn_front=%s dyn_rear=%s dyn_side=%s margin=%s\n' "$BODY_LENGTH_M" "$BODY_WIDTH_M" "$LIDAR_X_FROM_BODY_CENTER_M" "$DYNAMIC_FRONT_EXTRA_M" "$DYNAMIC_REAR_EXTRA_M" "$DYNAMIC_SIDE_EXTRA_M" "$SAFETY_MARGIN_M"
printf '[A1 START v6.8.2] front stop=%s turn_only=%s slow=%s half_deg=%s\n' "$FRONT_STOP_M" "$FRONT_TURN_ONLY_M" "$FRONT_SLOW_M" "$FRONT_SECTOR_HALF_DEG"

sed -i 's/\bsouce\b/source/g' "$HOME/.bash_profile" "$HOME/.bashrc" 2>/dev/null || true

echo "[A1 START v6.8.2] sudo check"
sudo -v

echo "[A1 START v6.8.2] stopping old processes"
...
echo "[A1 START v6.8.2] reset eth0"
...
echo "[A1 START v6.8.2] start scan provider"
nohup setsid bash --noprofile --norc -c "source /opt/ros/melodic/setup.bash; source $HOME/catkin_ws/devel/setup.bash; export ROS_MASTER_URI=http://localhost:11311; export ROS_IP=192.168.12.1; exec roslaunch $SCAN_LAUNCH_PKG $SCAN_LAUNCH_FILE" > "$LOG_DIR/scan_provider.log" 2>&1 < /dev/null &
echo $! > "$LOG_DIR/scan_provider.pid"

echo "[A1 START v6.8.2] waiting for /scan"
for i in $(seq 1 60); do
  ...
  sleep 1
  if [ "$i" = "60" ]; then
    echo "[A1 START v6.8.2] ERROR: /scan did not appear" >&2
    tail -160 "$LOG_DIR/scan_provider.log" >&2 || true
    exit 1
  fi
done

# Do NOT kill base_controller_node after roslaunch. On this A1 image, killing it
# makes roslaunch shut down the entire scan provider and ROS master, which
# causes /scan to become stale. It does not send Unitree HighCmd.

echo "[A1 START v6.8.2] check /scan message"
run_ros "timeout 8 rostopic echo -n 1 /scan" > "$LOG_DIR/scan_first_msg.log" 2>&1 || {
  echo "[A1 START v6.8.2] ERROR: /scan exists but no message" >&2
  tail -120 "$LOG_DIR/scan_first_msg.log" >&2 || true
  exit 1
}

echo "[A1 START v6.8.2] check /scan streaming"
run_ros "timeout 7 rostopic hz /scan" > "$LOG_DIR/scan_hz.log" 2>&1 || true
if ! grep -q "average rate" "$LOG_DIR/scan_hz.log"; then
  echo "[A1 START v6.8.2] ERROR: /scan exists but is not streaming" >&2
  echo "[A1 START v6.8.2] scan_hz.log:" >&2
  cat "$LOG_DIR/scan_hz.log" >&2 || true
  echo "[A1 START v6.8.2] scan_provider.log:" >&2
  tail -180 "$LOG_DIR/scan_provider.log" >&2 || true
  exit 1
fi
cat "$LOG_DIR/scan_hz.log"

echo "[A1 START v6.8.2] verify scan provider still alive"
if ! pgrep -f '[r]oslaunch.*slam_planner_online.launch' >/dev/null 2>&1; then
  echo "[A1 START v6.8.2] ERROR: roslaunch scan provider is not alive" >&2
  tail -180 "$LOG_DIR/scan_provider.log" >&2 || true
  exit 1
fi

echo "[A1 START v6.8.2] initialize command files"
...
if [ "$BODY_FILTER_MODE" = "pass" ]; then
  echo "[A1 START v6.8.2] BODY_FILTER_MODE=pass: direct passthrough, not recommended"
...
EOF
  ...
  echo "[A1 START v6.8.2] start body footprint filter monitor_only=$MONITOR_ONLY"
...
if [ "$CAM_OK" != "1" ]; then
  echo "[A1 START v6.8.2] ERROR: camera HTTP server not responding" >&2
  tail -160 "$LOG_DIR/camera_server.log" >&2 || true
  exit 1
fi

echo "[A1 START v6.8.2] start Unitree driver: $DRIVER_BIN"
...
if ! pgrep -f '[a]1_high_follow_driver' >/dev/null 2>&1; then
  echo "[A1 START v6.8.2] ERROR: Unitree driver is not running" >&2
  tail -120 "$LOG_DIR/high_follow_driver.log" >&2 || true
  exit 1
fi

echo "[A1 START v6.8.2] process summary"
ps aux | grep -E 'roslaunch|slamware_ros_sdk_server_node|slam_planner_node|base_controller_node|a1_lidar_footprint_filter_v6_8_2|a1_follow_lowlatency_depth_server|a1_high_follow_driver|a1_cmd_passthrough_loop' | grep -v grep || true

echo "[A1 START v6.8.2] status files"
echo "  logs: $LOG_DIR"
...
echo "[A1 START v6.8.2] DONE. Start PC client next."
