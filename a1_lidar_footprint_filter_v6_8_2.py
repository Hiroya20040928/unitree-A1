...
def main():
    rospy.init_node("a1_lidar_footprint_filter_v6_8_2")
    scan_topic = rospy.get_param("~scan_topic", "/scan")
    rospy.Subscriber(scan_topic, LaserScan, scan_cb, queue_size=1)
    rospy.loginfo("a1_lidar_footprint_filter_v6_8_2 started scan=%s raw=%s out=%s", scan_topic, RAW_CMD_PATH, OUT_CMD_PATH)
    ...
    while not rospy.is_shutdown():
        ...
        if en == 0 or st <= 0.0 or raw_age > raw_timeout:
            ...
            # LiDAR stream is stale. Do NOT write front=0.0, because the
            # Unitree driver interprets 0.0 m as a real front obstacle.
            # Stop final motion, but mark front distance as unknown/far so
            # logs expose the true cause as no_recent_scan_stop.
            write_front(999.0)
            write_cmd(0, 0.0, 0.0, 0.0)
